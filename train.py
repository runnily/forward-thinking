  ...
  def add_layers(self, change_epochs_each_layer=False, epochs_each_layer={}):
      ...
      if self.backpropgate == True:
        if self.get_loader != None:
          raise ValueError("You cannot backpropgate when there are different train sets defined for each layer")
...
class TrainWithDataSet(Train):
  ...
  def __init__(
    ...
    self.test_loader = DataLoader(test_dataset, batch_size=batch_size,num_workers=2, shuffle=True)
    self.get_loader = {}
