...
class Measure():
  ...
  def __init__(self,time_elapsed=[], epoch=[], loss=[], test_accuracy=[], train_accuracy=[]):
      ...
      self.__dict_vals = {"Epoch" : epoch , "Time Elapsed" : self.time_elapsed, 
                          "Loss" : self.loss, "Train accuracy" : self.train_accuracy, 
  ...
  def save(self, filename="accuracy.csv"):
    try:
      ...
      csv_rows = ["{},{},{},{},{}".format(x, y, z, i, k) for x, y, z, i, k in zip(*self.__vals)]
      csv_text += "\n".join(csv_rows)
