...
class Convnet2(BaseModel):
    def __init__(self, num_classes: int, batch_norm: bool, init_weights: bool = True) -> None:
        ...
        net = nn.Sequential(
          ...
          nn.Sequential(*conv_2d_relu(3, 64, (3, 3), batch_norm, padding=1)),
          # 1 : kernel: 3x3, channel: 64, padding: 1
          nn.Sequential(*conv_2d_relu(64, 64, (3, 3), batch_norm, padding=1)),
          # 2 : kernel: 3x3, channel: 64, padding: 1
          nn.Sequential(*conv_2d_relu(64, 128, (3, 3), batch_norm, padding=1),
                        nn.MaxPool2d(kernel_size=(2, 2), stride=2),
                        nn.Dropout(p=0.25)),
          # 3 : kernel: 3x3, channel: 128, padding: 1
          nn.Sequential(*conv_2d_relu(128, 128, (3, 3), batch_norm, padding=1)),
          # 4 : kernel: 3x3, channel: 128, padding: 1
          nn.Sequential(*conv_2d_relu(128, 256, (3, 3), batch_norm, padding=1),
                        nn.MaxPool2d(kernel_size=(2, 2), stride=2),
                        nn.Dropout(p=0.25)),
          # 5 : kernel: 3x3, channel: 256, padding: 1
          nn.Sequential(*conv_2d_relu(256, 256, (3, 3), batch_norm, padding=1)),
